...
@section('content')
    <h1>{{ Lang::get('emails.weekly-report.title', ['date' => $dates_title]) }}</h1>
    ...
    <h2>{{ Lang::get('emails.weekly-report.absent', ['count' => count($absent)]) }}</h2>

    <table class="table table-striped">
        <thead>
            <tr>
                <th>{{ Lang::get('emails.weekly-report.name') }}</th>
                <th>{{ Lang::get('emails.weekly-report.party') }}</th>
            </tr>
        </thead>
        <tbody>
    ...
    <h2>{{ Lang::get('emails.weekly-report.present', ['count' => count($present)]) }}</h2>
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th>{{ Lang::get('emails.weekly-report.name') }}</th>
                <th>{{ Lang::get('emails.weekly-report.party') }}</th>
                <th>{{ Lang::get('emails.weekly-report.weekly_minutes') }}</th>
                <th>{{ Lang::get('emails.weekly-report.weekly_hours') }}</th>
            </tr>
        </thead>
        <tbody>
