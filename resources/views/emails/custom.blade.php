...
@section('content')
    <h1>{{ Lang::get('emails.custom-report.title', ['date' => $dates_title]) }}</h1>
    ...
    <h2>{{ Lang::get('emails.custom-report.absent', ['count' => count($absent)]) }}</h2>
    ...
    <h2>{{ Lang::get('emails.custom-report.present', ['count' => count($present)]) }}</h2>
